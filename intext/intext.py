...
# json from ^^^pyjson^^^
import json
# random from ^^^pyrandom^^^
import random
# sys from ^^^pysys^^^
import sys
# pathlib from ^^^pypathlib^^^, using Path class is from ^^^pypathlibintro^^^
...
for p in paths:
    ...
    while True:
        ...
        # ^^^intext^^^ code would fail to be processed by InText as the break
        ...
        if not validKey:
            ...
            # from ^^^intext^^^ if continue were to occur as the beginning
